...
import type { STD_NXT_HRC2_NhapXuatTonRow } from "../../../models/STD_NXT_Model";
...
const isMismatch = (row: STD_NXT_HRC2_NhapXuatTonRow) => {
...
const renderKL = (value: number | null | undefined, row: STD_NXT_HRC2_NhapXuatTonRow) => {
...
const ThongKeNhapXuatTonHRC2 = () => {
  const [form] = Form.useForm();
  const [loading, setLoading] = useState(false);
  const [tableData, setTableData] = useState<STD_NXT_HRC2_NhapXuatTonRow[]>([]);
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const res = await STD_NXT_HRC2ServiceApi.searchNhapXuatTon({
      ...
      const rows: STD_NXT_HRC2_NhapXuatTonRow[] = Array.isArray(payload) ? payload : [];
      setTableData(rows);
      if (!rows.length) {
        message.info("Không có dữ liệu phù hợp với điều kiện lọc.");
      }
    } catch (error) {
      console.error("Lỗi thống kê Nhập Xuất Tồn HRC2:", error);
  ...
  const columns: ColumnsType<STD_NXT_HRC2_NhapXuatTonRow> = [
    {
    ...
    },
    {
      ...
      width: 130,
      align: "right",
      render: (_, row: STD_NXT_HRC2_NhapXuatTonRow) => {
        const { tonCuoiTruoc, tonDauSau } = row;
...
export default ThongKeNhapXuatTonHRC2;
